...
const Index = () => {
  ...
  return (
    <div className="min-h-screen app-bg">
      <div className={`${isMobile ? 'w-full px-3 py-4' : 'max-w-4xl p-4'} mx-auto min-h-screen flex flex-col`}>
        {/* 헤더 */}
        <div className={`text-center ${isMobile ? 'mb-4' : 'mb-6'}`}>
          <div className="flex justify-between items-start mb-4">
            <h1 className={`${isMobile ? 'text-sm leading-tight flex-1 mr-1' : 'text-3xl'} font-bold text-[hsl(var(--text-primary))] tracking-tight`}>MDEET 기출 중요 개념 암기카드</h1>
            <Button
              variant="outline"
              size={isMobile ? 'sm' : 'sm'}
              onClick={toggleTheme}
              className={`${isMobile ? 'p-2 min-w-0' : ''} bg-[hsl(var(--button-secondary))] text-[hsl(var(--button-secondary-text))] border-[hsl(var(--border-color))] hover:opacity-80`}
            >
              {theme === 'light' ? <Moon size={isMobile ? 14 : 16} /> : <Sun size={isMobile ? 14 : 16} />}
            </Button>
          </div>
        </div>
        ...
        {isMobile && filteredCards.length > 0 && (
          <div className="text-center mb-4">
            <Button 
              ...
              className="bg-[hsl(var(--button-primary))] text-[hsl(var(--button-primary-text))] hover:opacity-90 px-6 py-2 rounded-full font-semibold text-base"
            >
              학습 시작하기
            </Button>
          </div>
        ...
        <div className={`flex ${isMobile ? 'flex-col gap-3' : 'justify-between items-center'} mb-6`}>
          <div className={`flex ${isMobile ? 'flex-wrap gap-2' : 'gap-3'}`}>
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              <DialogTrigger asChild>
                <Button className={`flex items-center gap-2 bg-white text-gray-900 hover:bg-gray-100 shadow-lg ${isMobile ? 'px-4 py-2 text-sm' : 'px-6 py-3'} rounded-full font-medium`}>
            ...
            <Dialog open={isImportDialogOpen} onOpenChange={setIsImportDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" className={`flex items-center gap-2 bg-gray-800 text-gray-300 border-gray-600 hover:bg-gray-700 ${isMobile ? 'px-4 py-2 text-sm' : 'px-6 py-3'} rounded-full font-medium`}>
            ...
            {cards.length > 0 && (
              <Button 
                variant="outline" 
                onClick={exportToCSV}
                className={`flex items-center gap-2 bg-gray-800 text-gray-300 border-gray-600 hover:bg-gray-700 ${isMobile ? 'px-3 py-1 text-xs' : 'px-4 py-2'} rounded-full font-medium`}
              >
          ...
          {filteredCards.length > 0 && (
            <Button 
              onClick={() => setStudyMode(true)}
              className={`bg-white text-gray-900 hover:bg-gray-100 shadow-lg ${isMobile ? 'px-6 py-2 text-base' : 'px-8 py-3 text-lg'} rounded-full font-semibold`}
            >
              학습 시작
            </Button>
          )}
